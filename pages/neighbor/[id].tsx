...
const Home: NextPage<Props> = ({ allNeighborsData, neighborData }) => {
  return (
    <Layout>
      <Head>
        <title>{neighborData.build.toUpperCase()} - {neighborData.sc} {neighborData.name}的經營理念</title>
        <meta name="description" content={`富貴莊園第二屆管委員候選人${neighborData.name}的經營理念`} />

        <meta property="og:url" content={`https://fg2.vercel.app/neighbor/${neighborData.id}`} />
        <meta property="og:title" content={`${neighborData.build.toUpperCase()} - ${neighborData.sc} ${neighborData.name}的經營理念`} />
        <meta property="og:description" content={`富貴莊園第二屆管委員候選人${neighborData.name}的經營理念`} />
